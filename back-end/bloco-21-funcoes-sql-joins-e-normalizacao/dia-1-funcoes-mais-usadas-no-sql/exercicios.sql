...
SELECT MAX(SALARY) AS 'Maior salário' FROM employees;
...
SELECT JOB_ID, SUM(SALARY) AS `salario` FROM employees
	GROUP BY JOB_ID;
...
SELECT department_id, AVG(SALARY) AS media, COUNT(*) AS empregados FROM employees
...
SELECT UCASE(CONCAT(FIRST_NAME, ' ', LAST_NAME)) FROM employees ORDER BY EMPLOYEE_ID;
...
SELECT CONCAT(FIRST_NAME, ' ', LAST_NAME) AS nome, DATEDIFF(now(), HIRE_DATE) AS tempo_de_trabalho FROM employees
	GROUP BY nome;
